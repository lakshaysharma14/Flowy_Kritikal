...
const flowy = (canvas, grab, release, snapping, rearrange, spacing_x, spacing_y)=>
{
    if (!grab) 
    { grab = function() {}; }
    ...
    if (!snapping) 
    { snapping = function() { return true;}}

    if (!rearrange) 
    {  rearrange = function() { return false;    }}
...
const removeEventListenerMulti= (type, listener, capture, selector)=> {
    var nodes = document.querySelectorAll(selector);
    for (var i = 0; i < nodes.length; i++) {
...
export {flowy,addEventListenerMulti,removeEventListenerMulti};